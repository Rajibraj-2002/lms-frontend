...
const TopBar = () => {
            ...
            const fetchData = async () => {
                try {
                    const [notifRes, booksRes] = await Promise.all([
                        axios.get('https://lms-backend-production-d950.up.railway.app/api/user/my-notifications', { headers: { Authorization: `Bearer ${token}` } }),
                        axios.get('https://lms-backend-production-d950.up.railway.app/api/user/my-books', { headers: { Authorization: `Bearer ${token}` } })
                    ]);
    ...
    const handleBellClick = async (e) => {
        ...
        if (!showDropdown && unreadCount > 0) {
            setUnreadCount(0); 
            try {
                await axios.post('https://lms-backend-production-d950.up.railway.app/api/user/my-notifications/mark-read', {}, {
